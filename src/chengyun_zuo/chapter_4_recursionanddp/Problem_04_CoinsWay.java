package chengyun_zuo.chapter_4_recursionanddp;
...
public class Problem_04_CoinsWay {
	public static long min(int[] from, int[] to) {
		...
		int b=to.length;
		long sum=0;
		PriorityQueue help1=new PriorityQueue();
		PriorityQueue <Integer> help2 = new PriorityQueue<Integer>(11, new Comparator<Integer>() {
		...
		while(!help1.isEmpty() && !help2.isEmpty()){
			sum+=((long)help1.poll())*((long)help2.poll());
		}
		while(!help1.isEmpty()){
			sum+=((long)help1.poll())*((long)help1.poll());
		}
		while(!help2.isEmpty()){
			sum+=((long)help2.poll())*((long)help2.poll());
		}
		return sum;
	}
