...
/**
 * feature:汉诺塔问题比较经典，这里修改一下游戏规则：现在限制不能从最左侧的塔直接移动到最右侧，也不能从最右侧直接移动到最左侧，而是必须经过中间。
 * 求当塔有N层的时候，打印最优移动过程和最优移动步数
 *
 *例：当塔数为两层时，最上层的塔记为1，最下层的记为2，则打印：
 * 有 left、mid、right三个坑，如果2层则是1、2在left位置，然后需要移动到right位置上也就是1、2在right位置上
